  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (!subgraph) {
          response = await fetch(`https://${IP}/rnaprodb-backend/rnaprodb/run-script?pdbid=${pdbid}&algorithm=${algorithm}&isFirst=${isFirst}`);
        } else {
          response = await fetch(`https://${IP}/rnaprodb-backend/rnaprodb/run-script?pdbid=${pdbid}&subgraph=${subgraph}&algorithm=${algorithm}&isFirst=${isFirst}`);
        } 
